...
class DrawingGrid:
    def __init__(self, grid: Grid, length_square: int = 5, margin: int = 1,
    ...
    def create_random_maze(self) -> None:
        ...
        target_cell = self.grid.grid[int(MAZE_TARGET_ROW_FACTOR * self.grid.height)][int(MAZE_TARGET_COL_FACTOR * self.grid.height)]
        start_cell = self.grid.grid[int(MAZE_START_ROW_FACTOR * self.grid.height)][int(MAZE_START_COL_FACTOR * self.grid.width)]
