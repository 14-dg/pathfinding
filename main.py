"""
Einstiegspunkt des Programms.
Erstellt Grids, DrawingGrids und startet die Hauptschleife.
"""

import logging
from constants import *
from cell import Cell
from grid import Grid
from pathfinder import Pathfinder
from drawing_grid import DrawingGrid
from drawing_board import DrawingBoard


def main() -> None:
    logging.basicConfig(level=logging.INFO)

    g1 = Grid(200, 200, grid_name=MAIN_GRID)
    g2 = Grid(200, 200, grid_name=ROVER_GRID)
    g3 = Grid(200, 200, grid_name=SENSOR_GRID)
    g4 = Grid(200, 200, grid_name=ROVER_PATHFINDER_GRID)

    dg1 = DrawingGrid(g1, 2, 0, 0, 0)
    dg2 = DrawingGrid(g2, 2, 0, dg1.get_screen_dimensions()[0] + 10, 0)
    dg3 = DrawingGrid(g3, 2, 0, 0, dg1.get_screen_dimensions()[1] + 10)
    dg4 = DrawingGrid(g4, 2, 0, dg3.get_screen_dimensions()[0] + 10, dg1.get_screen_dimensions()[1] + 10)

    db = DrawingBoard(
        MAIN_GRID=dg1,
        ROVER_GRID=dg2,
        SENSOR_GRID=dg3,
        ROVER_PATHFINDER_GRID=dg4,
    )

    print("controls: ")
    print("'left click':    (can also hold down): set obstacle")
    print("'right click':   set starting point")
    print("'middle click':  remove cell to empty")
    print("'p' key:         set target point")
    print("'c' key:         clear board where mouse is")
    print("'m' key:         create random maze in ROVER_GRID and show LIDAR scan")
    print("'space' key:     remove all grids to empty")
    print("'enter' key:     run pathfinder")
    print("'j' key:         stop pathfinder and current path")
    print("'r' key:         reset pathfinder and clear pathfinding types")
    print("'i' key:         show debug info")
    print("'s' key:         save ROVER_GRID to saved_boards/rover_grid.txt")
    print("'l' key:         load ROVER_GRID from saved_boards/rover_grid.txt")
    print("Note: Pathfinder runs on ROVER_PATHFINDER_GRID but shows data from MAIN_GRID and ROVER_GRID")

    db.mainloop()


if __name__ == "__main__":
    try:
        main()
    finally:
        import pygame
        pygame.quit()